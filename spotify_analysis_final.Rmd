...
## Total number of songs from the top 5 artists
```{r}
spotify %>%
  count(artist_name) %>%
  top_n(5) %>%
...
## Acousticts and liveiness of all songs
```{r}
spotify %>%
  ggplot(aes(x = acousticness, y = liveness)) +
  geom_point(size=2.5) +
  geom_smooth(method = "lm", color="springgreen3") +
  labs(title = "Analysis of the Acousticts and Liveiness of All Songs",
       x = "Acousticness",
       y = "Liveness")+
  theme_minimal()
```
...
ui <- dashboardPage(skin = "green",
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Top 5 Artists", tabName = "top5"),
      menuItem("Pop Songs Table", tabName = "pop"),
      menuItem("Pop Artist Table", tabName = "pop_artist"),
      ...
      menuItem("Acousticts and Liveiness", tabName = "acoustic_liveness"),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "pop_artist",
              fluidRow(
                box(tableOutput("pop_artist"), width = 12)
              )
      ),
